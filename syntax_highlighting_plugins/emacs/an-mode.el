;;; an-mode.el --- Major mode for the Antha programming language

;; Copyright 2014 The Antha Authors. All rights reserved.
...
;; macro.
(if nil
    (declare-function go--position-bytes "an-mode" (point)))
...
(defconst an-mode-keywords
  '("break"    "default"     "func"   "interface" "select"
    "case"     "defer"       "go"     "map"       "struct"
    "chan"     "else"        "goto"   "package"   "switch"
    "const"    "fallthrough" "if"     "range"     "type"
    "continue" "for"         "import" "return"    "var"
    "Parameters" "Data" "Inputs" "Outputs" "Requirements" 
    "Controls" "Steps" "Analysis" "Validation")
  "All keywords in the Antha language.  Used for font locking.")
...
(defcustom an-mode-hook nil
  "Hook called by `an-mode'."
  :type 'hook
  :group 'go)
...
(defun go--build-font-lock-keywords ()
  ...
  (append
   `((,(go--regexp-enclose-in-symbol (regexp-opt an-mode-keywords t)) . font-lock-keyword-face)
   ...
   `(
     ...
     ;; Like the original an-mode this also marks compound literal
...
(defvar an-mode-map
  (let ((m (make-sparse-keymap)))
    (define-key m "}" #'an-mode-insert-and-indent)
    (define-key m ")" #'an-mode-insert-and-indent)
    (define-key m "," #'an-mode-insert-and-indent)
    (define-key m ":" #'an-mode-insert-and-indent)
    (define-key m "=" #'an-mode-insert-and-indent)
...
(defun an-mode-insert-and-indent (key)
...
;;;###autoload
(define-derived-mode an-mode prog-mode "Go"
...
\(https://github.com/dominikh/yasnippet-go)"
  ...
  ;; Indentation
  (set (make-local-variable 'indent-line-function) #'an-mode-indent-line)
...
;;;###autoload
(add-to-list 'auto-mode-alist (cons "\\.go\\'" 'an-mode))
...
Note that this will cause an-mode to get loaded the first time
you save any file, kind of defeating the point of autoloading."

  (interactive)
  (when (eq major-mode 'an-mode) (gofmt)))
