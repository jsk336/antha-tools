// antha-tools/oracle/describe.go: Part of the Antha language
...
func findInterestingNode(pkginfo *loader.PackageInfo, path []ast.Node) ([]ast.Node, action) {
	// TODO(adonovan): integrate with antha/types/stdlib_test.go and
	...
	for len(path) > 0 {
		...
		case *ast.Ident:
			...
			case *ast.ImportSpec:
				// TODO(adonovan): fix: why no package object? antha/types bug?
				return path[1:], actionPackage

			default:
				// e.g. blank identifier (antha/types bug?)
				// or y in "switch y := x.(type)" (antha/types bug?)
