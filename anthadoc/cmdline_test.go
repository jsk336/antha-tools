// antha-tools/anthadoc/cmdline_test.go: Part of the Antha language
...
package vet
`,
		"src/cmd/antha/doc.go": `// The antha command
package main
`,
...
{{comment_text .Doc "    " "\t"}}{{end}}{{end}}{{end}}`))
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			args: []string{"vet"},
			exp:  "use 'anthadoc cmd/vet' for documentation on the vet command \n\nPACKAGE Package vet\n",
		},
		{
			desc: "root directory",
			args: []string{"/"},
